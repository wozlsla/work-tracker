...
- Merge 자동 보관, 일반 커밋 보관·복원, 플래그 기반 중요 표시를 제공하는 구현 활동·중요·보관함 탭
...
- 16개 완성형 테마 프리셋, Tailwind Colors 기반 Custom 팔레트, 조합 가능한 Standard/Material 3 표면 스타일과 UI 글꼴
...
상단의 `Aa`에서 완성형 프리셋을 바로 적용할 수 있습니다. 선택한 프리셋과 글꼴은 브라우저에 저장되며 새로고침 후에도 유지됩니다.

- Material Theme 10종: Default, High Contrast, Darker High Contrast, Palenight, Palenight High Contrast, Ocean, Ocean High Contrast, Lighter, Lighter High Contrast, Darker
- Popular 6종: Absolutely, GitHub, Material, Nord, Tokyo Night, VS Code Plus
- Custom: Light/Dark/System 모드, Base surface, Accent, Standard/Material 3 surface, UI font를 독립적으로 조합

프리셋을 선택한 뒤 Base, Accent, Mode 또는 Surface style을 바꾸면 자동으로 Custom으로 전환됩니다. 글꼴은 프리셋과 독립적이어서 색상은 유지한 채 변경할 수 있습니다. Material Theme 계열은 [Material Theme](https://github.com/material-theme/vsc-material-theme), Nord와 Tokyo Night는 [Nord](https://github.com/nordtheme/visual-studio-code)와 [Tokyo Night](https://github.com/tokyo-night/tokyo-night-vscode-theme)의 공개 색상 정의를 기준으로 대시보드용 의미 색상에 매핑했습니다.
...
최초 한 번 또는 분석 데이터를 갱신할 때 `scan`을 실행합니다. 이후에는 `start.cmd` 한 줄만 실행하면 됩니다. 이 런처는 Python 3.11 이상을 자동 탐색하고 로컬 서버를 시작합니다. 브라우저에서 `http://127.0.0.1:8765/`를 열고, 서버를 중지할 때는 실행한 터미널에서 `Ctrl+C`를 누릅니다.
